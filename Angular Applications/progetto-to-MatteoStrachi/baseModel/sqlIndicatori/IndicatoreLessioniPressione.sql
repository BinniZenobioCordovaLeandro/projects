  ...
  select count(distinct(id_person)) as "persone con lesione"
   ...
   where id_contract in
     	 (
     	   select id
           from contracts c
          where ((valid_from between TO_DATE('10/11/2018', 'YYYY-MM-dd') and TO_DATE('25/12/2018', 'YYYY-MM-dd'))
             or (valid_from <= TO_DATE('10/11/2018', 'YYYY-MM-dd')
            and (valid_to is null or NULLIF(valid_to, TO_DATE('25/12/2018', 'YYYY-MM-dd'))
        between TO_DATE('10/11/2018', 'YYYY-MM-dd') and TO_DATE('25/12/2018', 'YYYY-MM-dd'))))
   ...
   where id_contract in
     	 (
      		  ...
      		  where id_service in
      		    ...
      		    and ((valid_from between TO_DATE('10/11/2018', 'YYYY-MM-dd') and TO_DATE('25/12/2018', 'YYYY-MM-dd'))
               or (valid_from <= TO_DATE('10/11/2018', 'YYYY-MM-dd')
              and (valid_to is null or NULLIF(valid_to, TO_DATE('25/12/2018', 'YYYY-MM-dd'))
          between TO_DATE('10/11/2018', 'YYYY-MM-dd') and TO_DATE('25/12/2018', 'YYYY-MM-dd'))))
              and id = f.id_contract
     ...
     and id_contract not in
       (
         select id
           from contracts c
          where valid_from between '10/11/2018' and '30/11/2018'
            and id_contractee in
          	  (
          	    select id
          	      from subjects_cr
...
(
  select count(*) as "persone con lesione, malnutrite, intervento nutrizionale (energetica - proteica)"
    from persons p
    where id in
        (
          select id_person
